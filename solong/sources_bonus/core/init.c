...
/*   Updated: 2024/06/20 10:43:51 by dgargant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_imgs(t_sprite *sprite, t_game *game)
{
	...
	sprite->cat = mlx_xpm_file_to_image(game->mlx,
			"./assets/Cat.xpm", &x, &y);
	sprite->cat2 = mlx_xpm_file_to_image(game->mlx,
			"./assets/Cat2.xpm", &x, &y);
	...
	sprite->egg = mlx_xpm_file_to_image(game->mlx,
			"./assets/Egg1.xpm", &x, &y);
	sprite->chiken = mlx_xpm_file_to_image(game->mlx,
			"./assets/Chiken1.xpm", &x, &y);
	sprite->chiken2 = mlx_xpm_file_to_image(game->mlx,
			"./assets/Chiken2.xpm", &x, &y);
...
void	init_game(t_game *game)
{
	...
	if (x < (game->w_length * 64) || y < (game->w_lines * 64))
		resolution_error(game);
	game->window = mlx_new_window(game->mlx, (game->w_length * 64),
			((game->w_lines - 1) * 64), "So_Long");
